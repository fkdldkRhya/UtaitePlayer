...
namespace UtaitePlayer.Classes.NAudioModule
{
    ...
    public class WaveOut : IWavePlayer, IWavePosition
    {
        ...
        /// <summary>
        /// Retrieves the capabilities of a RHYAWaveOut device
        /// </summary>
        ...
        private void Callback(IntPtr hWaveOut, WaveInterop.WaveMessage uMsg, IntPtr dwInstance, WaveHeader wavhdr, IntPtr dwReserved)
        {
            if (uMsg == WaveInterop.WaveMessage.WaveOutDone)
            {
                ...
                if (queuedBuffers == 0)
                {
                    if (callbackInfo.Strategy == WaveCallbackStrategy.FunctionCallback && playbackState == NAudio.Wave.PlaybackState.Stopped)
                    {
